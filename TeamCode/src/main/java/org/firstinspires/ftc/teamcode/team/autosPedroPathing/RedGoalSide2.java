...
@Autonomous(name = "Red Goal 9", group = "Pedro:Reds", preselectTeleOp = "TeleopFSM")
@Config
@Configurable
public class RedGoalSide2 extends DarienOpModeFSM {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        autoPlan = new AutoPlan()
                ...
                .add(new FollowPathStep(RedGoalSidePaths.buildShootingPosition2(follower), PATH_POWER_STANDARD, STANDARD_PATH_TIMEOUT))
                .add(new ShootSequenceStep(ShootingFSM.PowerLevel.CLOSE, SHOOT_TRIPLE_TIMEOUT))
                .add(new GateCloseStep())
                .add(new FollowPathStep(RedGoalSidePaths.buildIntakePos2(follower), PATH_POWER_STANDARD, STANDARD_PATH_TIMEOUT))
                .add(new IntakeStep(RedGoalSidePaths.buildIntakeBallSet2(follower), PATH_POWER_SLOW, STANDARD_PATH_TIMEOUT))
                .add(new FollowPathStep(RedGoalSidePaths.buildParking2Curve(follower), PATH_POWER_STANDARD, LONG_PATH_TIMEOUT, true))
                .add(new ShootSequenceStep(ShootingFSM.PowerLevel.CLOSE, SHOOT_TRIPLE_TIMEOUT))
                .add(new StopShotgunStep())
                .add(new CleanupStep());

        panelsTelemetry.debug("Status", "Initialized");
        telemetry.addLine("RedGoalSide2: READY");
        ...
        while (opModeIsActive() && !isStopRequested()) {
            ...
            addTraceTelemetry("Auto-RedGoalSide2", autoPlan.getStatus(), planTimer.getElapsedTimeSeconds());
            displayRpmTelemetry();
            panelsTelemetry.update(telemetry);
